...
const steps = [
  {
    ...
    description: 'فرد یا سازمان متوجه می‌شود که بین نتیجه مطلوب و نتیجه واقعی فاصله وجود دارد. مثال: کاهش فروش، افت بهره‌وری، نارضایتی مشتری.',
    icon: CircleAlert,
    color: 'bg-secondary',
  },
  {
    ...
    description: 'در ابتدا معمولاً تلاش می‌شود با تغییر اقدامات یا رفتارها، مشکل برطرف شود؛ بدون اینکه فرض‌های بنیادی زیر سؤال برود. مثال: افزایش تبلیغات برای جبران کاهش فروش.',
    icon: RefreshCw,
    color: 'bg-primary',
  },
  {
    ...
    description: 'در این مرحله می‌پرسیم: چرا این مشکل رخ داده است؟ چه باورها، سیاست‌ها یا فرض‌هایی باعث این وضعیت شده‌اند؟ آیا اهداف، استراتژی‌ها یا روش‌های فعلی هنوز معتبرند؟ اینجا تفاوت اصلی با یادگیری تک‌حلقه‌ای شکل می‌گیرد.',
    icon: Brain,
    color: 'bg-accent-2',
  },
  {
    ...
    description: 'فرض‌ها، ارزش‌ها، قواعد و سیاست‌های حاکم بازطراحی یا اصلاح می‌شوند. مثال: به‌جای فقط افزایش تبلیغات، شرکت متوجه می‌شود که مدل ارزش پیشنهادی محصول دیگر با نیاز بازار همخوان نیست و باید استراتژی محصول تغییر کند.',
    icon: LayoutGrid,
    color: 'bg-accent-3',
  },
  {
    ...
    description: 'بر اساس چارچوب جدید، اقدامات و تصمیمات تازه اجرا می‌شوند. مثال: بازطراحی محصول، تغییر استراتژی بازار، یا تغییر ساختار سازمانی.',
    icon: Rocket,
    color: 'bg-accent',
  },
  {
    ...
    description: 'نتایج اقدامات جدید ارزیابی می‌شود و سازمان دوباره از تجربه‌ها یاد می‌گیرد. این چرخه به یادگیری عمیق و تحول سازمانی منجر می‌شود.',
    icon: BarChart3,
    color: 'bg-accent-4',
  },
]
...
<template>
  <section class="w-full px-4 py-16 lg:px-12 lg:py-28">
    <div class="mx-auto flex max-w-6xl flex-col items-center justify-center gap-4">
      ...
      <p class="mx-auto mb-16 max-w-lg text-start font-vazir text-base leading-relaxed text-muted-foreground">
